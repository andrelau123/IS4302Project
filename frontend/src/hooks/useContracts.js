    ...
    if (provider && hasValidFeeDistributor && hasValidAuthToken) {
      try {
        ...
        if (
          ...
          ProductRegistryABI?.abi?.length > 0
        ) {
        ...
        if (CONTRACT_ADDRESSES.PRODUCT_NFT && ProductNFTABI?.abi?.length > 0) {
...
export const useProductRegistry = () => {
  ...
  const transferProduct = useCallback(
    async (productId, to, location = "") => {
      ...
      try {
        const contractWithSigner = contract.connect(signer);
        const tx = await contractWithSigner.transferProduct(
          productId,
          to,
          location
        );
...
export const useProductNFT = () => {
  ...
  const mintNFT = useCallback(
    async (to, productId, metadataURI) => {
      if (!contract || !signer || !isConnected) {
        toast.error("Please connect your wallet first");
        return null;
      }
