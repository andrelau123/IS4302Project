 ...
 * @notice Accepts signed IoT/human attestations and aggregates them (weighted quorum) per verification request.
 * @dev Designed to be read by VerificationManager before finalizing a verification.
 */
contract OracleIntegration is AccessControl, ReentrancyGuard, Pausable, EIP712 {
    ...
    event SourceRegistered(address indexed signer, SourceType sType, uint8 weight);
    ...
    event AggregationParamsUpdated(uint256 quorumWeight, uint256 passBpsThreshold);
    ...
    function registerSource(address signer, SourceType sType, uint8 weight) external onlyRole(ORACLE_ADMIN_ROLE) {
    ...
    function updateSource(address signer, bool active, uint8 weight) external onlyRole(ORACLE_ADMIN_ROLE) {
        require(sources[signer].sType == SourceType.IoT || sources[signer].sType == SourceType.Human, "Not registered");
        require(weight > 0, "Weight=0");
    ...
    function setAggregationParams(uint256 _quorumWeight, uint256 _passBpsThreshold) external onlyRole(ORACLE_ADMIN_ROLE) {
    ...
    //submission
    /**
     * @notice Submit a signed attestation from a registered source (IoT or Human).
     * @dev Uses EIP-712 typed data. Enforces deadline and per-signer nonce (replay protection).
     */
    ...
    /**
     * @notice (Optional) Allow a trusted system component to push attestations on-chain without signatures.
     * @dev Grant SUBMITTER_ROLE only to your own backend relayer if you truly need this.
     */
    ...
    /**
     * @notice Returns whether quorum is met and if the (weighted) pass ratio clears the threshold.
     * @return quorumReached true if totalWeight >= quorumWeight
     * @return passed true if (passWeight / totalWeight) >= passBpsThreshold
     * @return totalWeight sum of weights submitted
     * @return passWeight sum of weights with verdict==true
     * @return count number of unique attestations used
     */
    function getAggregate(bytes32 requestId)
        external
        view
        returns (bool quorumReached, bool passed, uint256 totalWeight, uint256 passWeight, uint256 count)
    {
        ...
        passed = (totalWeight == 0) ? false : (passWeight * 10_000 / totalWeight) >= passBpsThreshold;
    }

    /// @notice (Optional) Prevent further attestations for a request once your VerificationManager has read it.
    ...
    /// @notice Check if a given source has already attested to requestId.
    function hasSourceAttested(bytes32 requestId, address sourceAddr) external view returns (bool) {
        return aggregates[requestId].seen[sourceAddr];
    }

    // helpers
    function _recover(Attestation calldata a, bytes calldata sig) internal returns (address signer) {
